...
def get_square(lat,lon):
    ...
    lat = int(round(lat))
    lon = int(round(lon))
    for x in range(lat,2*lat):
        pointsmap.append((lon,x))
        pointsmap.append((2*lon-1,x))
    for y in range(lon,2*lon):
        pointsmap.append((y,lat))
        pointsmap.append((y,2*lat-1))
...
def get_traclet(trajectory,size,max_speed):
    ...
    if min_lon != max_lon and min_lat != max_lat:
        ...
        for index, p in trajectory.iterrows():
            ...
            for y_x in get_square(scale(min_lon,max_lon,p['longitude'],int(round((size-1)/2))),scale(min_lat,max_lat,p['latitude'],int(round((size-1)/2)))):
...
makedirs('traclets_sqr_image', exist_ok=True)
...
for t in trajectories:
    ...
    makedirs(f'traclets_sqr_image/{label}', exist_ok=True)
    start = datetime.datetime.now()
    traclet = get_traclet(t,size,max_speed)
    end = datetime.datetime.now()
    cv2.imwrite(f'traclets_sqr_image/{label}/{id}.png', traclet)
    delta = end - start
